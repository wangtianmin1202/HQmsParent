...
import com.hand.plm.spec_product_detail.util.dto.FormHeadDto;
    ...
    public static <E> void writeText(HSSFWorkbook workbook, HSSFSheet sheet, FormTextDto formTextDto, List<E> list, Map map) throws Exception {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            for (Field field : fields) {
                ...
                Object getValue = get.invoke(list.get(i), new Object[]{});
                if (titleMap.get(field.getName()) != null) {
                    ...
                    if (getValue == null) {
                        ...
                        if (getValue instanceof BigDecimal) {
                            ...
                            cell.setCellValue(Double.parseDouble(getValue.toString()));
                            style2.setDataFormat(df.getBuiltinFormat("#,##0.00"));//保留两位小数点
                        }else if (getValue instanceof Integer) {
                            int value = ((Integer) getValue).intValue();
                            cell.setCellValue(value);
                            style2.setDataFormat(df.getBuiltinFormat("#,#0"));//数据格式只显示整数
