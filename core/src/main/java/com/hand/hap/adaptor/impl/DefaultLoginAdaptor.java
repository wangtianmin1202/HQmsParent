...
public class DefaultLoginAdaptor implements ILoginAdaptor {
	...
	public ModelAndView indexView(HttpServletRequest request, HttpServletResponse response) {
		...
		Boolean b = true;
		try {
			b = CheckUserAndIpPro(request, (Long) session.getAttribute(User.FIELD_USER_ID));
		} catch (Exception e) {
			b = true;
		}
		if (!b) {
			User user = new User();
			user.setUserId((Long) session.getAttribute(User.FIELD_USER_ID));
			user.setUserName((String) session.getAttribute(User.FIELD_USER_NAME));
			return loginView(request, response);
		}
		...
		if (session != null) {
			...
			if (userName != null) {
				if (session.getAttribute(User.LOGIN_CHANGE_INDEX) != null) {
					...
					List<IUserSecurityStrategy> userSecurityStrategies = userSecurityStrategyManager
							.getUserSecurityStrategyList();
			...
			if (!sysConfigManager.getRoleMergeFlag()) {
				...
				List<IRole> roles = roleService.selectActiveRolesByUser(RequestHelper.createServiceRequest(request),
						user);
	 ...
	 * @return
	 */
	private Boolean CheckUserAndHost(HttpServletRequest request, Long userId) {
		Boolean successLogin = true;
		String host = request.getHeader("Host");
		User cUser = new User();
		cUser.setUserId(userId);
		User userInfo = userService.queryUserById(cUser);
		if (!StringUtils.isEmpty(userInfo.toString()) && userInfo.getUserType().toUpperCase().equals("I")) {
			if (!StringUtils.isEmpty(host)) {
				IRequest requestContext = RequestHelper.createServiceRequest(request);
				List<CodeValue> codeValueList = codeService.selectCodeValuesByCodeName(requestContext, "SYS.HOST");
				for (CodeValue codeValue : codeValueList) {
					if (codeValue.getEnabledFlag().equals("Y")
							&& codeValue.getMeaning().toUpperCase().equals(host.toUpperCase())) {
						successLogin = false;
					}
				}
			}
	 ...
	 * @return
	 */
	private boolean CheckUserAndIp(HttpServletRequest request, Long userId) throws IOException {
		Boolean successLogin = true;
		String ip = null;
		URL ipUrl = new URL("http://httpbin.org/ip");
		...
		if (!StringUtils.isEmpty(userInfo.toString()) && userInfo.getUserType().toUpperCase().equals("I")) {
			if (!StringUtils.isEmpty(ip)) {
				...
				List<CodeValue> codeValueList = codeService.selectCodeValuesByCodeName(requestContext, "SYS.HOST");
				for (CodeValue codeValue : codeValueList) {
					if (codeValue.getEnabledFlag().equals("Y") && !ip.contains(codeValue.getMeaning())) {
						successLogin = false;
					}
				}
			}
	 ...
	 * @return
	 */
	private boolean CheckUserAndIpPro(HttpServletRequest request, Long userId) {
		Boolean successLogin = false;// 默认不可登录

		RedisSingle redisSingle = RedisSingle.getInstance();
		String ip = redisSingle.strGet("loginip");

		User cUser = new User();
		cUser.setUserId(userId);
		User userInfo = userService.queryUserById(cUser);
		if (!StringUtils.isEmpty(userInfo.toString()) && !userInfo.getUserType().toUpperCase().equals("S")) {
			if (!StringUtils.isEmpty(ip)) {
				IRequest requestContext = RequestHelper.createServiceRequest(request);
				List<CodeValue> codeValueList = codeService.selectCodeValuesByCodeName(requestContext, "SYS.HOST");
				for (CodeValue codeValue : codeValueList) {
					if (codeValue.getEnabledFlag().equals("Y") && codeValue.getMeaning().equals(ip)) {
						successLogin = true;
					}
				}
			}
