...
public class CodeServiceImpl extends BaseServiceImpl<Code> implements ICodeService {
    ...
    @Override
    @Transactional(propagation = Propagation.SUPPORTS)
    public List<CodeValue> selectCodeValuesByCodeNameTag(IRequest request, String codeName,String tag) {
        //Code code = getValue(request, codeName);
        //if (code != null) {
            //return code.getCodeValues();
        //} else {
            return codeValueMapper.selectCodeValuesByCodeNameTag(codeName,tag);
        //}
    }
